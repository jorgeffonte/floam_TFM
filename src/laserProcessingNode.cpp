...
// #include  <floam_msgs/PclPlusTime.h>
...
bool setMaxDistance(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res) {
    if (req.data) {
        laserProcessing.setMaxDistance(100.0);  // Example distance
        res.success = true;
        res.message = "Max distance set to 100.0";
    } else {
        laserProcessing.setMaxDistance(60.0);  // Default distance
        res.success = true;
...
void laser_processing(){
    while(1){
        if(!pointCloudBuf.empty()){
            ...
            ROS_INFO("current laser processing time %f ms", time_temp);
            ...
            *pointcloud_filtered+=*pointcloud_surf;
            ...
            msg.point_cloud = edgePointsMsg;
            pubEdgePoints.publish(msg);
...
int main(int argc, char **argv)
{
    ...
    //if lidar_topic is not set, use default value
    if(lidar_topic==""){
    ...
    ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(lidar_topic, 1, velodyneHandler);

    pubLaserCloudFiltered = nh.advertise<sensor_msgs::PointCloud2>("/velodyne_points_filtered", 100);

    pubEdgePoints = nh.advertise<floam::PclPlusTime>("/laser_cloud_edge", 100);
